...
| Permission denied                   | Local mode: need `transform-custom:*` — see Prerequisites -> IAM Permissions in [custom](custom.md). Remote mode: generate and attach policies via `npx ts-node generate-caller-policy.ts` — see [custom-remote-execution](custom-remote-execution.md) |
...
If missing, guide the user through setup — see the Collect Repositories step in [custom](custom.md).
